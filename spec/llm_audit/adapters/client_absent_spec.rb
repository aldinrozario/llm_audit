...
# The one spec file that exercises the adapter's absent path on the client-absent CI leg
# (gemfiles/no_llm_gems.gemfile), and the reason that leg is not vacuous: every other spec file that loads
# the client does it through spec/support/ruby_llm_client.rb and is therefore in that leg's
# --exclude-pattern, while this one is deliberately not - so this is the only run in which the adapter's
# absent path meets a client that is genuinely not installed rather than one that is merely not required.
...
RSpec.describe "the client-absent path" do
  ...
  # suite that exercises the adapter with the client genuinely absent - llm_audit_spec.rb's other
  # subprocesses lack the client too, but none of them reaches an adapter - so the readers are shown
